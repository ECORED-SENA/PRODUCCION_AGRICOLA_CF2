<template lang="pug">
.curso-main-container.pb-3
  BannerInterno
  .container.tarjeta.tarjeta--blanca.p-4.p-md-5

    .titulo-principal
      .titulo-principal__numero
        .h3 1
      .h3 Sistemas de gestión de la seguridad y salud en el trabajo

    figure.mb-5
      img(src='@/assets/curso/tema1/img01.png', alt='Sistemas de gestión de la seguridad y salud en el trabajo')

    .row.mb-5.justify-content-center
      .col-10
        .bloque-texto-b.p-4
          .bloque-texto-b__texto.h4.mb-0 La Seguridad y Salud en el Trabajo -SST- es la disciplina que trata la prevención de las lesiones y enfermedades causadas por las condiciones de trabajo, y de la protección y promoción de la salud de los trabajadores

          p (Ministerio del Trabajo, s.f.).

    p.mb-4 Según la Ley 1562 de 2012  el Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST busca con la participación de varias disciplinas y con el compromiso activo de todos los niveles de la organización optimizar las condiciones de trabajo y de salud de la población trabajadora, mediante acciones coordinadas de promoción de estilos de trabajo y vida saludables, la prevención y el control de los factores de riesgos, de manera que faciliten el bienestar de la comunidad laboral y la eficiencia del Ministerio.

    p.mb-4 Así como también el SG-SST busca garantizar que en el lugar de trabajo todos y cada uno de los operarios tengan el conocimiento necesario para realizar las labores, además que cuenten con el equipo adecuado para que haya una práctica segura y que, en el imprevisto de algún accidente, se cuente con lo mínimo para ser atendidos oportunamente, desde la capacitación como contar con los elementos necesarios. 

    .row.mb-5.justify-content-center
      .col-6
        p.mb-4 “La Seguridad y Salud en el Trabajo -SST- es la disciplina que trata la prevención de las lesiones y enfermedades causadas por las condiciones de trabajo, y de la protección y promoción de la salud de los trabajadores” (Ministerio del Trabajo, s.f.).

        p Según la Ley 1562 de 2012  el Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST busca con la participación de varias disciplinas y con el compromiso activo de todos los niveles de la organización optimizar las condiciones de trabajo y de salud de la población trabajadora, mediante acciones coordinadas de promoción de estilos de trabajo y vida saludables, la prevención y el control de los factores de riesgos, de manera que faciliten el bienestar de la comunidad laboral y la eficiencia del Ministerio.

      .col-6
        figure
          img(src='@/assets/curso/tema1/img02.svg', alt='Sistemas de gestión de la seguridad y salud en el trabajo')
    
    .titulo-segundo
      .h4 1.1 Normatividad general
    
    p.mb-4 Dentro del marco normativo del Sistema de Gestión de Seguridad y Salud en el Trabajo se encuentran leyes, decretos y resoluciones que orientan en los diferentes procesos sobre la salud y el bienestar de los trabajadores, considerando que la responsabilidad de adoptar las medidas para garantizar la salud ocupacional de las personas trabajadoras en el lugar de trabajo corresponde a cada unidad productiva del sector agrícola.

    LineaTiempoB.mb-5(:datos="datosLineaTiempoB")

    .titulo-segundo
      .h4 1.2 Campo de aplicación

    .row.mb-5.justify-content-center
      .col-6
        p.mb-4 El SG-SST debe contener y manifestar, de forma clara, cómo la producción agrícola o la unidad productiva se compromete a minimizar los riesgos de accidentes que se identifiquen en las tareas o actividades llevadas a cabo, tanto con los trabajadores como operarios temporales. En este, se debe tener en cuenta el personal, el factor de riesgo, la descripción del factor de riesgo, posibles medidas de control, los daños y su severidad del daño, probabilidades de ocurrencia, así como también categorizar los riesgos y contar con las medidas de control para los riesgos identificados a través del diagnóstico realizado.
      
      .col-6
        figure
          img(src='@/assets/curso/tema1/img03.svg', alt='Campo de aplicación')

    p.mb-4 El SG-SST debe incluir un plan de capacitación para los trabajadores y operarios en el que se incluyan temas como el de la señalización de las áreas, equipos de protección personal, extintores, botiquín, instructivos de emergencia y primeros auxilios, manejo seguro de plaguicidas, accidentes entre otros, buscando, así mismo, su bienestar y por ende, se tienen en cuenta espacios como comedores, baterías sanitarias, lavamanos, disponibilidad de agua para beber; lo cual estará a cargo de la persona de salud, seguridad y bienestar del trabajador encargado de hacer un seguimiento.

    .row.mb-5.justify-content-center
      .col-10
        figure
          img(src='@/assets/curso/tema1/img04.svg', alt='Campo de aplicación')
     
    p.mb-5 De acuerdo con las BPA la o las personas capacitadas en primeros auxilios (mínimo una, en la producción agrícola o finca) debe de permanecer en esta en el momento de llevar a cabo las labores para atender una posible emergencia. Además, en la implementación de BPA se encuentra la inocuidad de los productos como pilar fundamental, por ello es indispensable contar con la información básica sobre la higiene en la que se incluye: comer y beber en áreas designadas, mantener las manos limpias, usar los trajes o equipos de protección personal de acuerdo con la labor, y avisar cualquier problema de salud o infección que se pueda presentar. 

    .titulo-segundo
      .h4 1.3. Estándares mínimos según el tipo de empresa
    
    p Entre las ventajas que tiene el SG-SST es la de mejorar el ambiente de trabajo, el bienestar y la calidad de vida laboral, así como también la reducción de accidentalidad y mortalidad, ausentismo del puesto de trabajo debido a enfermedades que se puedan presentar en el momento de las labores y, cómo contribuir al aumento de la producción agrícola. También es importante tener en cuenta que el SG-SST es el encargado de velar por el cumplimiento de las normas, requisitos y procedimientos de las empresas y contratantes en riesgos laborales. 

    p.mb-4 A continuación, puede revisar en la tabla los Estándares Mínimos del Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST. MinTrabajo

    .h5 Tabla 1

    p Estándares mínimos del Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST

    .tabla-a
    table
      caption Nota: tomado del Capítulo I, Artículo 3, Resolución 0312 del 13 de febrero de 2019. Estándares mínimos del Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST. Ministerio del Trabajo (2019)
      thead
        tr
          th Ítem
          th Criterios, empresas de diez o menos trabajadores clasificados en riesgo I, II, III
          th Modo de verificación
      tbody
        tr
          td Asignación de persona que diseña el Sistema de Gestión de SST
          td Asignar una persona que cumpla con el siguiente perfil:
            p.mb-4  El diseño del Sistema de Gestión de Seguridad y Salud en el Trabajo para empresas de menos de diez trabajadores en clase de riesgo I, II, III puede ser realizado por un técnico en Seguridad y Salud en el Trabajo (SST) o en alguna de sus áreas con licencia vigente en Seguridad y Salud en el Trabajo, que acredite mínimo un año de experiencia certificada por las empresas o entidades en las que laboró en el desarrollo de actividades de Seguridad y Salud en el Trabajo y que acredite la aprobación del curso de capacitación virtual de cincuenta horas.
            p Esta actividad también podrá ser desarrollada por tecnólogos, profesionales y profesionales con posgrado en SST, que cuenten con licencia vigente en Seguridad y Salud en el Trabajo y el referido curso de capacitación virtual de cincuenta horas. 
          td Solicitar documento soporte de la asignación y constatar la hoja de vida con soportes, de la persona asignada. 
        tr
          td Asignación al Sistema de Seguridad y Salud Integral
          td Afiliación a los Sistemas de Seguridad Social en Salud, Pensión y Riesgos Laborales de acuerdo con la normatividad vigente.
          td Solicitar documento soporte de afiliación y del pago correspondiente.
        tr
          td Asignación de persona que diseña el Sistema de Gestión de SST
          td Asignar una persona que cumpla con el siguiente perfil:
          td Solicitar documento soporte de la asignación y constatar la hoja de vida con soportes, de la persona asignada. 

    .row.mb-5.justify-content-center
      .col-5
        a.anexo.mb-4.mb-lg-0(href="https://www.youtube.com/watch?v=qeaZcyVeJWo" target="_blank")
          .anexo__icono
            img(src="@/assets/template/icono-link.svg")
          .anexo__texto
            p Para profundizar observe el video “Modificación estándares mínimos de SG-SST en Colombia”

    .titulo-segundo
      .h4 1.4. Peligros asociados a la producción agrícola
    
    p.mb-4 A partir de la definición de peligro “fuente, situación o acto con potencial de causar daño en la salud de los trabajadores, en los equipos o en las instalaciones” (Decreto 1072 de 2015, Art. 2.2.4.6.2), se relacionan algunos de los peligros en las labores de las unidades productivas o en la empresa agrícola:

    .row.mb-5
      .col-sm.mb-5.mb-sm-0
        ul.lista-ul
          li 
            i.fas.fa-angle-right
            |	Condiciones de temperatura y humedad.
          li 
            i.fas.fa-angle-right
            |	Radiaciones no ionizadas generadas por el sol.
          li 
            i.fas.fa-angle-right
            |	Ruido
          li 
            i.fas.fa-angle-right
            |	Insectos (gusanos, hormigas, avispas, entre otras).
          li 
            i.fas.fa-angle-right
            |	Ausencia de agua potable y servicios sanitarios
          li 
            i.fas.fa-angle-right
            |	Fertilizantes de síntesis química.
          li 
            i.fas.fa-angle-right
            |	Plaguicidas.
          li 
            i.fas.fa-angle-right
            |	Topografía irregular del terreno.
          li 
            i.fas.fa-angle-right
            |	Drenajes y hoyos.
          li 
            i.fas.fa-angle-right
            |	Suelos resbaladizos o presencia de escombros y otros objetos o materiales.
          li 
            i.fas.fa-angle-right
            |	Hacinamiento.
      .col-sm.mb-5.mb-sm-0
        ul.lista-ul
          li 
            i.fas.fa-angle-right
            |	Deficientes condiciones de orden y limpieza.
          li 
            i.fas.fa-angle-right
            |	Tractores y sus aditamentos.
          li 
            i.fas.fa-angle-right
            |	Maquinaria.
          li 
            i.fas.fa-angle-right
            |	Escaleras portátiles.
          li 
            i.fas.fa-angle-right
            |	Herramientas (manuales, filosas o punzocortantes, mecánicas o de motor).
          li 
            i.fas.fa-angle-right
            |	Falta de señalización.
          li 
            i.fas.fa-angle-right
            |	Desplazamientos horizontales
          li 
            i.fas.fa-angle-right
            |	Levantamientos de cargas y objetos.
          li 
            i.fas.fa-angle-right
            |	Movimientos repetitivos de hombro y brazos.
          li 
            i.fas.fa-angle-right
            |	Permanecer de pie.
          li 
            i.fas.fa-angle-right
            |	Jornada extraordinaria de trabajo.
          li 
            i.fas.fa-angle-right
            |	Intensidad en el ritmo de trabajo.

    .titulo-segundo
      .h4 1.5. Riesgos asociados a la producción agrícola
    
    p.mb-4 Buena parte del trabajo agrícola es, por naturaleza, físicamente exigente. El riesgo de accidentes aumenta con el cansancio, la utilización de herramientas poco ergonómicas, las dificultades del terreno, la exposición a condiciones climáticas extremas y la mala salud general que se asocia a trabajar y vivir en comunidades rurales remotas. Estos problemas se ven agravados por el hecho de que las condiciones de trabajo y de vida terminan solapándose.  En términos de fallecimientos, lesiones y problemas de salud relacionados con el trabajo, el sector agrícola es uno de los tres sectores de actividad más peligrosos junto con la construcción y la minería (OIT, 2015).

    .row.mb-5.justify-content-center
      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk01.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo físico
              p Están relacionados factores ambientales que actúan sobre los tejidos y órganos del cuerpo de la persona trabajadora, que pueden producir efectos nocivos, de acuerdo con la intensidad y tiempo de exposición. Entre las labores se encuentran: preparación del terreno, siembra y resiembra, mantenimiento del cultivo, cosecha, manejo integrado de plagas, postcosecha.

      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk02.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo biológico
              p Son agentes orgánicos como los hongos, virus, bacterias, parásitos, insectos presentes en determinados ambientes laborales, que pueden desencadenar enfermedades infectocontagiosas, reacciones alérgicas o intoxicaciones al ingresar al organismo. Entre las labores se encuentran: preparación del terreno, siembra y resiembra, mantenimiento del cultivo, cosecha, manejo integrado de plagas, postcosecha.

      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk03.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo químico
              p Toda sustancia orgánica, inorgánica, natural o sintética que durante la fabricación, manejo, transporte, almacenamiento y uso pueda incorporarse al aire ambiente y ser inhalada; entrar en contacto con la piel o ser ingerida, con efectos irritantes, corrosivos, asfixiantes o tóxicos y en cantidades o tiempos de exposición que tengan probabilidades de lesionar la salud de las personas.  Entre las labores se encuentran: manejo integrado de plagas, enfermedades y arvenses de los cultivos, cosecha y postcosecha.
      
      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk04.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo locativo
              p Cuando se trata de construcción, mantenimiento, topografía del terreno y deterioro de instalaciones locativas. Entre las labores se encuentran: preparación del terreno y postcosecha.

      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk05.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo mecánico
              p Factores laborales por la acción mecánica de máquinas, equipos, herramientas o materiales proyectados sólidos o fluidos que pueden conllevar a producir un accidente. Entre las labores se encuentran: preparación del terreno, siembra, resiembra, aplicación de plaguicidas, cosecha, postcosecha.

      .col-lg-10
        .tarjeta-Avatar-b.mb-5
            img(src='@/assets/curso/tema1/risk06.svg' alt='AvatarTop')
            .tarjeta.tarjeta--azul.p-4
              .h4 Riesgo ergonómico
              p Son aquellos que se producen cuando el trabajador se encuentra en su lugar de trabajo y en este, se hacen movimientos, acciones o posturas que producen daño a la salud. Entre las labores se encuentran todas aquellas que corresponden al desarrollo del cultivo.

    .titulo-segundo
      .h4 1.6. Lesiones por accidentes de trabajo asociados a la producción agrícola
    
    p.mb-4 Cuando se habla de implementación de BPA en la empresa agrícola es frecuente que se produzcan accidentes laborales de trabajadores en las labores que desempeñan diariamente en las unidades productivas. A continuación, se listan algunas de las lesiones que se encuentran después de sufrir un accidente por las labores realizadas en la unidad productiva o empresa agrícola.

    figure.mb-5
      img(src='@/assets/curso/tema1/img05.svg', alt='Lesiones por accidentes de trabajo asociados a la producción agrícola')
    
    .row.mb-5
      .col-sm.mb-5.mb-sm-0
        ul.lista-ul
          li 
            i.fas.fa-angle-right
            |	Electrocución con línea eléctrica.
          li 
            i.fas.fa-angle-right
            |	Intoxicación aguda o crónica por plaguicidas y otros productos químicos.
          li 
            i.fas.fa-angle-right
            |	Cortes con tijeras o herramientas neumáticas, heridas punzocortantes. 
          li 
            i.fas.fa-angle-right
            |	Lesiones por motosierra o por utilización de otros aperos agrícolas.
          li 
            i.fas.fa-angle-right
            |	Quemaduras de piel. 
          li 
            i.fas.fa-angle-right
            |	Disminución de capacidad auditiva.
          li 
            i.fas.fa-angle-right
            |	Disminución en la productividad y aumento de ausentismo. 
      .col-sm.mb-5.mb-sm-0
        ul.lista-ul
          li 
            i.fas.fa-angle-right
            |	Alergias, picaduras, mordeduras o muerte por otros organismos vivos. 
          li 
            i.fas.fa-angle-right
            |	Irritación en los ojos y vías respiratorias. 
          li 
            i.fas.fa-angle-right
            |	Ardor en la garganta, la nariz, los ojos o la piel. 
          li 
            i.fas.fa-angle-right
            |	Malestar general: mareo, náuseas, desmayo, vómito, temblores, dolor de cabeza, entre otros. 
          li 
            i.fas.fa-angle-right
            |	Resbalones, golpes, quebraduras, caídas, heridas, fracturas, contusiones, amputaciones. 
          li 
            i.fas.fa-angle-right
            |	Sobre esfuerzo físico, lesiones músculo esqueléticas, fatiga. 
          li 
            i.fas.fa-angle-right
            |	Trastornos del sueño, pérdida del apetito, depresión, ansiedad, irritabilidad, insatisfacción, disminución de la capacidad de trabajo.

</template>

<script>
import Muestras from '../components/Muestras' // borrar una vez el componente "Muestras" no se necesite
export default {
  name: 'Tema1',
  components: {
    Muestras, // borrar una vez el componente "Muestras" no se necesite
  },
  data: () => ({
    datosLineaTiempoB: [
      {
        titulo: 'El Código Sustantivo del Trabajo de 1951',
        texto:
          'Busca lograr la justicia en las relaciones que surgen entre empleadores y trabajadores, dentro de un espíritu de coordinación económica y equilibrio social. En su Capítulo 2 trata el tema de accidentes de trabajo y enfermedades y en su Título XI abarca el tema de higiene y seguridad en el trabajo, algunos de sus artículos fueron derogados por el Decreto 1295 de 1994 y la Ley 962 de 2005. Este código regula las relaciones de derecho individual del trabajo de carácter particular, y las de derecho colectivo del trabajo, oficiales y particulares y rige en todo el territorio de la República para todos sus habitantes, excepto los servidores públicos, sin consideración de su nacionalidad.',
        icono: require('@/assets/curso/tema1/code.svg'),
      },
      {
        titulo: 'Ley 9 de enero 24 de 1979',
        texto:
          'Es una ley muy importante ya que hasta la fecha sigue vigente, llamada Código Sanitario Nacional, da los lineamientos con respecto a la preservación, conservación y mejoramiento de la salud de los individuos en sus ocupaciones.',
        icono: require('@/assets/curso/tema1/law.svg'),
      },
      {
        titulo: 'Resolución 2013 de junio 6 de 1986',
        texto:
          'Del Ministerio de Trabajo y Seguridad Social y Ministerio de Salud, por la cual se reglamenta la organización y funcionamiento de los comités paritarios de salud ocupacional en los lugares de trabajo (COPASO), hoy llamado comités paritarios de seguridad y salud en el trabajo (COPASST).',
        icono: require('@/assets/curso/tema1/resolution.svg'),
      },
      {
        titulo: 'Decreto Ley 1295 de junio 22 de 1994',
        texto:
          'Expedido por el Ministerio de Trabajo y Seguridad Social, por el cual se determinan la organización y administración del Sistema General de Riesgos Profesionales.',
        icono: require('@/assets/curso/tema1/decret.svg'),
      },
      {
        titulo: 'Ley 776 de diciembre 17 de 2002',
        texto:
          'Por la cual se dictan normas sobre la organización, administración y prestaciones del Sistema General de Riesgos Profesionales.',
        icono: require('@/assets/curso/tema1/law.svg'),
      },
      {
        titulo: 'Decreto 2090 de julio 26 de 2003',
        texto:
          'Por el cual se definen las actividades de alto riesgo para la salud del trabajador y se modifican y señalan las condiciones, requisitos y beneficios del régimen de pensiones de los trabajadores que laboran en dichas actividades.',
        icono: require('@/assets/curso/tema1/decret.svg'),
      },
      {
        titulo: 'Ley 54 de 12 de diciembre 2003',
        texto:
          'De reforma del marco normativo de la Prevención de Riesgos Laborales.',
        icono: require('@/assets/curso/tema1/law.svg'),
      },
      {
        titulo: 'Decreto 1072 de 2015',
        texto: 'Decreto Único Reglamentario del Sector Trabajo.',
        icono: require('@/assets/curso/tema1/decret.svg'),
      },
      {
        titulo: 'Decreto 1496 de agosto 6 de 2018',
        texto:
          'Por el cual se adopta el Sistema Globalmente Armonizado de Clasificación y Etiquetado de Productos Químicos y se dictan otras disposiciones en materia de seguridad química.',
        icono: require('@/assets/curso/tema1/decret.svg'),
      },
      {
        titulo: 'Resolución 0312 de 2019',
        texto:
          'Estándares mínimos del Sistema de Gestión de Seguridad y Salud en el Trabajo SG-SST.',
        icono: require('@/assets/curso/tema1/resolution.svg'),
      },
    ],
  }),
}
</script>

<style lang="sass" scoped></style>
